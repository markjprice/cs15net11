...
A client makes an HTTP request to a resource, such as a page, uniquely identified by a URL, and the server sends back an HTTP response, as shown in *Figure 12.6*:
...
- **Scheme**: `http` (clear text) or `https` ("secure"/encrypted).
