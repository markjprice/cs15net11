...
To enable JSON Patch support with System.Text.Json, use the Microsoft.AspNetCore.JsonPatch.SystemTextJson NuGet package. This package provides a JsonPatchDocument<T> class to represent a JSON Patch document for objects of the T type and custom logic for serializing and deserializing JSON Patch documents using System.Text.Json. The key method of the JsonPatchDocument<T> class is ApplyTo, which applies the patch operations to a target object of the T type:
...
Warning! The new System.Text.Json implementation of JSON Patch isn't a complete drop-in replacement for the existing Newtonsoft.Json-based implementation. In particular, the new implementation doesn't support dynamic types such as ExpandoObject, which you learned about in Chapter 2, Speaking C#.
