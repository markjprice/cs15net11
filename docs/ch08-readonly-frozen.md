...
Generic collections, like `List<T>` and `Dictionary<TKey, TValue>`, have an `AsReadOnly` method to create a `ReadOnlyCollection<T>` instance that references the original collection. Although `ReadOnlyCollection<T>` must have `Add` and `Remove` methods because it implements `ICollection<T>`, it throws a `NotSupportedException` to prevent changes.
...
7.	In `Program.cs`, comment out the `AsReadOnly` statement and then add a statement to pass the keywords converted into an immutable dictionary:
```cs
//UseDictionary(keywords.AsReadOnly());
...
There are two frozen collections: `FrozenDictionary<TKey, TValue>` and `FrozenSet<T>`. More may come in future versions of .NET, but these are the two most common scenarios that would benefit from the frozen concept.
...
- `ImmutableList<T>`: Returns a new list with the new item in it without affecting the original list.
- `ImmutableDictionary<TKey, TValue>`: Returns a new dictionary with the new item in it without affecting the original dictionary.
- `FrozenDictionary<TKey, TValue>`: `Add` does not exist.
